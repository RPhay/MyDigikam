 ...
 *
 * In strict technical terms, this is not a classifier.  This is a post-processor.
...
{
	/// @note: This classifier does not handle YOLO v10
    ...
    if (yoloVersion == YOLOv5 || yoloVersion == YOLOv7)
	{
		...
		outputNumbox = 3 * (imageSize.width() / 8 * imageSize.height() / 8 + imageSize.width() / 16 * imageSize.height() / 16 + imageSize.width() / 32 * imageSize.height() / 32);
	}
	if (yoloVersion == YOLOv6)
	{
		outputNumprob = 5 + predefinedClasses.size();
		outputNumbox = imageSize.width() / 8 * imageSize.height() / 8 + imageSize.width() / 16 * imageSize.height() / 16 + imageSize.width() / 32 * imageSize.height() / 32;
	}
...
QList<int> AutotagsClassifierYolo::predictMulti(const QList<cv::Mat>& targets)  const
{
    ...
    for (int i = 0; i < outputNumbox; ++i)
	{
		...
		cv::Rect box;
		if(yoloVersion == YOLOv5 || yoloVersion == YOLOv6 || yoloVersion == YOLOv7 || yoloVersion == YOLOv8 || yoloVersion == YOLOv9 || yoloVersion == YOLOv11)
		{
			float x = ptr[0];
			float y = ptr[1];
			float w = ptr[2];
	...
	if(yoloVersion == YOLOv5 || yoloVersion == YOLOv6 || yoloVersion == YOLOv7 || yoloVersion == YOLOv8 || yoloVersion == YOLOv9 || yoloVersion == YOLOv11)
	{
		...
		for (int i = 0; i < (int)indices.size(); i++)
		{
			...
			if (saveResult)
			{
				qCDebug(DIGIKAM_AUTOTAGSENGINE_LOG) << "New cls - Class ID: " << class_ids[idx] << " " << predefinedClasses[class_ids[idx]] << " Confidence: " << scores[idx] << " Objness: " << objnesses[idx];
			}
			else
			{
				qCDebug(DIGIKAM_AUTOTAGSENGINE_LOG) << "New cls - Class ID: " << class_ids[idx] << " " << predefinedClasses[class_ids[idx]] << " Confidence: " << scores[idx] << " Objness: " << objnesses[idx] << " below threshold";
			}
		}
	}
...
// #include "moc_autotagsclassifiermultiyolo.cpp"
