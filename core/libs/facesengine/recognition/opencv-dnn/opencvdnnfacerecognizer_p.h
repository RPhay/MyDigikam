...
bool OpenCVDNNFaceRecognizer::Private::insertData(const cv::Mat& nodePos, const int label, const QString& hash)
{
    if (nodePos.rows != 1)
    {
    ...
    int nodeId = FaceDbAccess().db()->insertFaceVector(nodePos, label, hash);
