...
bool GPCamera::doConnect()
{
    ...
    int modelNum = gp_abilities_list_lookup_model(abilList, model().toLatin1().constData());
    int portNum  = gp_port_info_list_lookup_path(infoList, port().toLatin1().constData());
...
bool GPCamera::getMetadata(const QString& folder, const QString& itemName, DMetadata& meta)
{
    ...
    if (!exifData.isEmpty())
    {
        ...
        if (i != -1)
        {
            ...
            QByteArray ba;
            ba.resize(exifData.size() - i);
            memcpy(ba.data(), exifData.data() + i, ba.size());
            meta.setExif(ba);
