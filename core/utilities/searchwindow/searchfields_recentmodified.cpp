...
void SearchFieldRecentModified::applySeconds(int seconds)
{
    ...
    const Unit units[] = { {86400}, {3600}, {60}, {1} };
...
{
    if (m_amountBox) m_amountBox->setValue(0);
    if (m_unitCombo) m_unitCombo->setCurrentIndex(-1);
    setValidValueState(false);
}

void SearchFieldRecentModified::setValueWidgetsVisible(bool visible)
{
    if (m_amountBox) m_amountBox->setVisible(visible);
    if (m_unitCombo) m_unitCombo->setVisible(visible);
}

QList<QRect> SearchFieldRecentModified::valueWidgetRects() const
{
    QList<QRect> rects;
    if (m_amountBox) rects << m_amountBox->geometry();
    if (m_unitCombo) rects << m_unitCombo->geometry();
    return rects;
}
