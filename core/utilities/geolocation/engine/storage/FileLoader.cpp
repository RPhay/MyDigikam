...
private:
    ...
    FileLoaderPrivate(const FileLoaderPrivate&)            = delete;
...
void FileLoader::run()
{
    ...
    if (d->m_contents.isEmpty())
    {
        ...
        qCDebug(DIGIKAM_GEOENGINE_LOG) << "starting parser for" << d->m_filepath;
        ...
        if (QFile::exists(defaultSourceName))
        {
            qCDebug(DIGIKAM_GEOENGINE_LOG) << "No recent Default Placemark Cache File available!";
        ...
        {
            qCDebug(DIGIKAM_GEOENGINE_LOG) << "No Default Placemark Source File for " << name;
        }
    ...
    else
    {
        ...
        {
            qCWarning(DIGIKAM_GEOENGINE_LOG) << QString::fromUtf8("Could not import kml buffer!");
        ...
        qCDebug(DIGIKAM_GEOENGINE_LOG) << "newGeoDataDocumentAdded" << d->m_filepath;
...
void FileLoaderPrivate::documentParsed(GeoDataDocument* doc, const QString& error)
{
    ...
    if (doc)
    {
        ...
        if (m_renderOrder != 0)
        {
            for (GeoDataPlacemark* const placemark : doc->placemarkList())
            {
                if (GeoDataPolygon* const polygon = geodata_cast<GeoDataPolygon>(placemark->geometry()))
                {
                    ...
                    )
            {
                ...
                if (diameter >= 0)
                {
                    ...
                    if (placemarkRole == QLatin1String("c"))
                    {
                        ...
                        if (
                            (placemark->name() == QLatin1String("Tycho")) ||
            ...
            else if (placemarkRole == QLatin1String("PPLC"))
            {
                switch (placemark->zoomLevel())
                {
                    ...
                    default:
                    {
                     ...
                     (placemarkRole == QLatin1String("PPLA2")) ||
