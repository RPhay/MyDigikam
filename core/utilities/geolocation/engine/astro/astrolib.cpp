...
int Eclipse::solar(double jd, double tdut, double& phi, double& lamda)
{
    ...
    if (r0 < 1.0)
    {
        ...
        if (d_umbra > 0)
        {
            phase = 4;    // central annular eclipse
        }

        else
        {
            phase = 5;    // central total eclipse
        }
    }
    ...
    else
    {
        if (r0 < (1.0 + 0.5 * fabs(d_umbra)))
        {
            if (d_umbra > 0)
            {
                phase = 2;    // non-central annular eclipse
            }

            else
            {
                phase = 3;    // non-central total eclipse
            }
        }
...
double Eclipse::duration(double jd, double tdut, double& width)
{
    ...
    */
    ...
    if (fabs(pa) < 1.0e-15)
    {
        lm = fabs(d_umbra);    // just in case
    }

    else
    {
        lm = fabs(d_umbra / pa);    // projection along shadow vector
    }
...
int Eclipse::lunar(double jd, double tdut)
{
    ...
    else if (sep < (umbra + dm / 2.0))
    {
        phase = 3;
    }

    else if (sep < (penumbra - dm / 2.0))
    {
        phase = 2;
    }
