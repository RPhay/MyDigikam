...
void PlanetarySats::setSatFile(const char* const fname)
{
...
int PlanetarySats::getStateVector(int nsat)
{
    ...
    if (searching)
    {
        while (searching)
        {
            ...
            if (fsc)
            {
                if (strncmp(pls_plntname, plntname, 4) == 0)
                {
                    nst++;
                }
                ...
                if (nst == nsat)
                {
                    ...
                    if (utc)
                    {
                        pls_tepoch = pls_tepoch + pls_del_tdut / 86400.0;    // epoch in TT
                    }
                };
            };
        };
...
void PlanetarySats::setPlanet(const char* const pname)
{
    pls_moonflg = false;
    ...
    if (strncmp("Venus", pname, 4) == 0)
    {
        getVenus();
    }

    if (strncmp("Mercury", pname, 4) == 0)
    {
        getMercury();
    }
...
void PlanetarySats::stateToKepler()
{
    ...
    {
        n = 1.0;    // do something to avoid a domain error
    }
    ...
    if (n > 1000.0)
    {
        n = 1000.0;    // avoid possible errors
    }
...
int PlanetarySats::selectSat(const char* const sname)
{
    ...
    while (searching)
    {
        ...
        if (res)
        {
            if (strncmp(pls_satname, sname, sl) == 0)
            {
                searching = false;
            }
        }
...
void PlanetarySats::getSatPos(double tutc)
{
    ...
    if (a < 1.0)
    {
        a = 1.0;    // avoid possible crashes later on
    }
    ...
    if (pls_moonflg)
    {
        m1 = getSelenographic(t);
    }

    else
    {
        m1 = zrot((pls_W + pls_Wd * (t - 51544.5)) * (M_PI / 180.0));
    }
    ...
    {
        pls_height = 0;    // this should never happen
    }
