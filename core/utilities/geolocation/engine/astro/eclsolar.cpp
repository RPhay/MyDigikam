...
int EclSolar::getLocalTotal(double& mjd_start, double& mjd_stop)
{
    ...
    if (eb_lunactive)
    {
        ...
        if (mjd_start < eb_lcb1)
        {
            mjd_start = eb_lcb1;
        }

        if (mjd_start > eb_lce1)
        {
            k = 0;
        }
...
void EclSolar::getEclYearInfo(char* wbuf)
{
    ...
    for (j = 0; j < eb_numecl; j++)
    {
        ...
        switch (p)
        {
            ...
            case 2:
                strcat(outbuf, "\t non-central Annular Sun");
                break;

            case 4:
                strcat(outbuf, "\t Annular Sun");
                break;

            case 3:
                strcat(outbuf, "\t non-central Total Sun");
                break;

            case 5:
                strcat(outbuf, "\t Total Sun");
                break;

            case 6:
                strcat(outbuf, "\t Annular/Total Sun");
                break;
...
int EclSolar::getEclTxt(int k, char* jtxt)
{
    ...
    switch (p)
    {
        ...
        case -1:
        case -2:
            strcat(jtxt, " Pen.Moon");
            break;

        case -3:
            strcat(jtxt, " Par.Moon");
            break;

        case -4:
            strcat(jtxt, " Tot.Moon");
            break;
    };
...
void EclSolar::previousEcl()
{
    ...
    {
        putEclSelect(0);    // this case should never happen!
    }
                ...
                if (fabs(gam) <= (1.5432 + u))
                {
                    if (fabs(gam) < 0.9972)    // central eclipse
                    {
                        ...
                        else
                        {
                            if (u > 0.0047)
                            {
                                eph = 4;    // annular eclipse
                            }

                            else
                            {
                                if (u < (0.00464 * cos(asin(gam))))
                                {
                                    eph = 6;    // annular/total
                                }
        ...
        if (pcur > pold)
        {
            ...
            if (nump < 4)
            {
                ...
                for (i = 0; i < 60; i++)
                {
                    ...
                    {
                        pcur = eclp.solar(jd2, eb_del_tdut, d1, d2);
                    }
...
void EclSolar::calcMaxPos(double& lat, double& lng)
{
    ...
    {
        p = eclp.solar(t, eb_del_tdut, lat, lng);
    }
...
void EclSolar::InitBound()
{
    ...
    */
    ...
    if (eb_lunactive)
    {
        return;    // only for solar eclipses
    }
    ...
    {
        dpn1 = 1.2 * abs(eb_ubm);    // avoid a crash
    }
...
int EclSolar::GNSBound(bool firstc, bool north, double& lat, double& lng)
{
    ...
    {
        vre = eb_lbe + (t - eb_jdstart) * eb_lde;
    }
...
int EclSolar::GRSBound(bool firstc, double& lat1, double& lng1, double& lat2, double& lng2)
{
    ...
    */
    ...
    if ((r0 > fabs(1.0 - dpn)) && (r0 < fabs(1.0 + dpn)))
    {
        ...
        if (iscrs(vrc[0], vrc[1], dpn, vrx[0], vrx[1], vrx2[0], vrx2[1]))
        {
            lat1 = 0;
        }
        ...
        {
            if (iscrs(vrc[1], vrc[0], dpn, vrx[1], vrx[0], vrx2[1], vrx2[0]))
            {
                lat1 = 0;
            }
        };
    ...
    if (maxp > 0)
    {
        for (i = 0; i < maxp; i++)
        {
            if (p < 0)
            {
                ...
                {
                    case 1:
                        strcpy(outb, "penumbral   ");
                        break;

                    case 2:
                        strcpy(outb, "tot.penumb  ");
                        break;
            ...
            else
            {
                switch (spp[i])
                {
                    ...
                    case 2:
                        strcpy(outb, "n-centr.Ann ");
                        break;

                    case 3:
                        strcpy(outb, "n-centr.Tot ");
                        break;

                    case 4:
                        strcpy(outb, "annularity  ");
                        break;
...
void EclSolar::getLocalDetails(char* otxt)
{
    ...
    {
        case 1:
            strcpy(outb, "\t\tPartial Eclipse of the Sun");
            break;

        case 2:
            strcpy(outb, "\t\tNon-Central Annular Eclipse of the Sun");
            break;

        case 3:
            strcpy(outb, "\t\tNon-Central Total Eclipse of the Sun");
            break;

        case 4:
            strcpy(outb, "\t\tAnnular Eclipse of the Sun");
            break;

        case 5:
            strcpy(outb, "\t\tTotal Eclipse of the Sun");
            break;

        case 6:
            strcpy(outb, "\t\tAnnular/Total Solar Eclipse");
            break;

        case -1:
        case -2:
            strcpy(outb, "\t\tPenumbral Eclipse of the Moon");
            break;

        case -3:
            strcpy(outb, "\t\tPartial eclipse of the Moon");
            break;

        case -4:
            strcpy(outb, "\t\tTotal eclipse of the Moon");
            break;
    };
    ...
    if (ecloutbn == 3)
    {
        ...
        for (i = 0; i < nump; i++)
            if ((spt[i] < jd) && (spp[i] > 3))
            {
                jd = spt[i];    // start of central phase
            }
        ...
        for (i = 0; i < nump; i++)
            if ((ept[i] > jdf) && (spp[i] > 3))
            {
                jdf = ept[i];    // end of central phase
            }
    }
...
double EclSolar::navCourse(double lat1, double lng1, double lat2, double lng2)
{
    ...
    if (an == 0)
    {
        return 0;    // same spot. didn't move
    }
...
void EclSolar::getShadowCone(double mjd, bool umbra, int numpts, double* lat, double* lng)
{
    ...
    */
    ...
    if (umbra && (eb_phase[eb_eclselect - 1] < 1))
    {
        return;
    }
    ...
    {
        eclp.penumd(mjd, eb_del_tdut, ubm, ube, dpn1, pan1);
    }
    ...
    if (pan1 < 0.0000174533)
    {
        return;    // if cone is smaller that 0.001°
    }
    ...
    if ((kmiss < 2) || (kmiss >= (numpts - 1)))
    {
        return;    // cone completely on Earth surface or not at all
    }
