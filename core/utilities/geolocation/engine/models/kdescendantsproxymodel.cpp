...
public:
    ...
    explicit KDescendantsProxyModelPrivate(KDescendantsProxyModel* qq)
        : q_ptr                             (qq),
          m_rowCount                        (0),
...
QModelIndexList KDescendantsProxyModel::match(const QModelIndex& start, int role,
...
void KDescendantsProxyModel::setSourceModel(QAbstractItemModel* _sourceModel)
{
    ...
    if (sourceModel())
    {
        ...
        {
            disconnect(sourceModel(), modelSignals[i],
                       this, proxySlots[i]);
        }
    }
    ...
    if (_sourceModel)
    {
        for (int i = 0 ; i < int(sizeof modelSignals / sizeof * modelSignals) ; ++i)
        {
            connect(_sourceModel, modelSignals[i],
                    this, proxySlots[i]);
        }
    }
