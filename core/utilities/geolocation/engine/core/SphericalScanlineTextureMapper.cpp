    ...
    for (int y = m_yTop; y < m_yBottom ; ++y)
    {
        ...
        for (int x = xLeft; x < xRight; ++x)
        {
            ...
            if (x >= xIpLeft && x <= xIpRight)
            {
                ...
                //                qCDebug(DIGIKAM_GEOCORE_LOG) << QString("NorthPole X: %1, LeftInterval: %2").arg( northPoleX ).arg( leftInterval );
                if (crossingPoleArea
            ...
            //            qCDebug(DIGIKAM_GEOCORE_LOG) << QString("lon: %1 lat: %2").arg(lon).arg(lat);
